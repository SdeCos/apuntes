...
def taylor_method(f, tmin, tmax, h, y0, n, ecuacion_original):
    ...
    # Realizar la primera iteración paso a paso
    print("\nPrimera iteración paso a paso:")
    i = 0
    print("===================================")
    t_0 = t_values[0]
    y_0 = y_values[0]
    print(f"Paso {i+1}: t_{i} = {t_0}, y_{i} = {y_0}")
    ...
    # Fórmula de Taylor para la primera iteración
    wi = y_0 + h * f_value + (h**2 / 2) * f_prime_y_value
    print(f"\nCálculo de w_1: w_1 = y_0 + h * f(t_0, y_0) + (h^2 / 2) * f'_y(t_0, y_0)")
    print(f"w_1 = {y_0} + {h} * {f_value} + ({h**2} / 2) * {f_prime_y_value} = {wi}")
    print(f"w_1 = {wi}\n")

    # Guardar el valor calculado de w_1
    y_values[1] = wi
